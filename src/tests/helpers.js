...
export const fillOutForm = async () => {
  ...
  await userEvent.type(cpfInput, expectedCpf);
  await userEvent.type(addressInput, expectedAddress);
  await userEvent.type(cityInput, expectedCity);
  await userEvent.selectOptions(stateSelect, expectedState);

  await userEvent.click(apRadioButton);

  await userEvent.type(resumeTextArea, expectedResume);
  await userEvent.type(roleInput, expectedRole);
  await userEvent.type(roleDescriptionTextArea, expectedRoleDescription);
};
